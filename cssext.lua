...
local requestsDisabled = true --getgenv and getgenv().DISABLE_GameXsan_REQUESTS
local InterfaceBuild = '3K3W'
local Release = "Build 1.68"
local GameXsanFolder = "GameXsan"
local ConfigurationFolder = GameXsanFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		GameXsanOpen = {Type = 'bind', Value = 'K', Name = 'GameXsan Keybind'},
		-- buildwarnings
		-- GameXsanprompts
...
local overriddenSettings: { [string]: any } = {} -- For example, overriddenSettings["System.GameXsanOpen"] = "J"
...
local prompt = useStudio and require(script.Parent.prompt) or loadWithTimeout('https://raw.githubusercontent.com/donitono/versi1/refs/heads/main/prompt.lua')
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(GameXsanFolder) then
				if isfile and isfile(GameXsanFolder..'/settings'..ConfigurationExtension) then
					file = readfile(GameXsanFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"GameXsanOpen":{"Value":"K","Type":"bind","Name":"GameXsan Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"GameXsan Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('GameXsan had an issue accessing configuration saving capability.')
		end
	end
end
...
local GameXsan = useStudio and script.Parent:FindFirstChild('GameXsan') or game:GetObjects("rbxassetid://10804731440")[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local GameXsanDestroyed = false -- True when GameXsanLibrary:Destroy() is called

repeat
	if GameXsan:FindFirstChild('Build') and GameXsan.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('GameXsan | Build Mismatch')
		print('GameXsan may encounter issues as you are running an incompatible interface version ('.. ((GameXsan:FindFirstChild('Build') and GameXsan.Build.Value) or 'No Build') ..').\n\nThis version of GameXsan is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, GameXsan = GameXsan, useStudio and script.Parent:FindFirstChild('GameXsan') or game:GetObjects("rbxassetid://10804731440")[1]
	if toDestroy and not useStudio then toDestroy:Destroy() end

	buildAttempts = buildAttempts + 1
until buildAttempts >= 2

GameXsan.Enabled = false

if gethui then
	GameXsan.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(GameXsan)
	GameXsan.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	GameXsan.Parent = CoreGui:FindFirstChild("RobloxGui")
...
FloatingButton.Name = "GameXsanFloating"
...
FloatingFrame.Position = UDim2.new(0, 25, 0.5, 0)
FloatingFrame.Size = UDim2.new(0, 25, 0, 25)
...
FloatingButtonFrame.TextSize = 8
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == GameXsan.Name and Interface ~= GameXsan then
			Interface.Enabled = false
			Interface.Name = "GameXsan-Old"
		end
	end
elseif not useStudio then
...
local minSize = Vector2.new(280, 160) -- Extra compact for mobile landscape
local useMobileSizing

if GameXsan.AbsoluteSize.X < minSize.X and GameXsan.AbsoluteSize.Y < minSize.Y then
...
local Main = GameXsan.Main
local MPrompt = GameXsan:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = GameXsan:FindFirstChild('Drag')
...
GameXsan.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/SiriusSoftwareLtd/GameXsan/refs/heads/main/icons.lua')
-- Variables
...
local Notifications = GameXsan.Notifications

local SelectedTheme = GameXsanLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = GameXsanLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	GameXsan.Main.BackgroundColor3 = SelectedTheme.Background
	GameXsan.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	GameXsan.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	GameXsan.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	GameXsan.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	GameXsan.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	GameXsan.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		GameXsan.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		GameXsan.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(GameXsan:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("GameXsan | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("GameXsan | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('GameXsan had an issue decoding the configuration file, please try delete the file and reopen GameXsan.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(GameXsanLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("GameXsan | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--GameXsanLibrary:Notify({Title = "GameXsan Flags", Content = "GameXsan was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(GameXsanLibrary.Flags) do
...
function GameXsanLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('GameXsan | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			GameXsanLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
		else
			GameXsanLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {getSetting("General", "GameXsanOpen")}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function Maximise()
	...
	TweenService:Create(Main, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {Size = useMobileSizing and UDim2.new(0, 280, 0, 160) or UDim2.new(0, 350, 0, 250)}):Play()
	TweenService:Create(Topbar, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {Size = UDim2.new(0, 350, 0, 18)}):Play()
...
local function Unhide()
	...
	Main.Visible = true
	TweenService:Create(Main, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {Size = useMobileSizing and UDim2.new(0, 280, 0, 160) or UDim2.new(0, 350, 0, 250)}):Play()
	TweenService:Create(Main.Topbar, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {Size = UDim2.new(0, 350, 0, 18)}):Play()
...
local function saveSettings() -- Save settings to config file
	...
	if success then
		...
		if writefile then
			writefile(GameXsanFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('GameXsan Settings', 0, true)

	if TabList['GameXsan Settings'] then
		TabList['GameXsan Settings'].LayoutOrder = 1000
	end

	if Elements['GameXsan Settings'] then
		Elements['GameXsan Settings'].LayoutOrder = 1000
	end
...
function GameXsanLibrary:CreateWindow(Settings)
	print('creating window')
	if GameXsan:FindFirstChild('Loading') then
		if getgenv and not getgenv().GameXsanCached then
			GameXsan.Enabled = true
			GameXsan.Loading.Visible = true

			task.wait(1.4)
			GameXsan.Loading.Visible = false
		end
	end

	if getgenv then getgenv().GameXsanCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				GameXsanLibrary:Notify({Title = 'Build Mismatch', Content = 'GameXsan may encounter issues as you are running an incompatible interface version ('.. ((GameXsan:FindFirstChild('Build') and GameXsan.Build.Value) or 'No Build') ..').\n\nThis version of GameXsan is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	if Settings.ToggleUIKeybind then -- Can either be a string or an Enum.KeyCode
		...
		if type(keybind) == "string" then
			...
			overrideSetting("General", "GameXsanOpen", keybind)
		elseif typeof(keybind) == "EnumItem" then
			assert(keybind.EnumType == Enum.KeyCode, "ToggleUIKeybind must be a KeyCode enum")
			overrideSetting("General", "GameXsanOpen", keybind.Name)
		else
			error("ToggleUIKeybind must be a string or KeyCode enum")
		end
	end

	if isfolder and not isfolder(GameXsanFolder) then
		makefolder(GameXsanFolder)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "GameXsan"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "GameXsan Interface Suite" then
		LoadingFrame.Version.Text = "GameXsan UI"
	end
	...
	if not Settings.DisableGameXsanPrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				GameXsanLibrary:Notify({
					Title = "GameXsan Interface",
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		if isfolder and not isfolder(GameXsanFolder.."/Discord Invites") then
			makefolder(GameXsanFolder.."/Discord Invites")
		end

		if isfile and not isfile(GameXsanFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if requestFunc then
				pcall(function()
					requestFunc({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(GameXsanFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"GameXsan RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(GameXsanFolder.."/Key System") then
			makefolder(GameXsanFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("GameXsan | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with GameXsan specific development.')
				end
			end
		end
		...
		if isfile and isfile(GameXsanFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(GameXsanFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(GameXsanFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						GameXsanLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		TabButton.Size = UDim2.new(0, TabButton.Title.TextBounds.X + 15, 0, 20)
		...
		if Image and Image ~= 0 then
			...
			TabButton.Size = UDim2.new(0, TabButton.Title.TextBounds.X + 30, 0, 20)
		end
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				-- Prevents animation from trying to play if the button's callback called GameXsanLibrary:Destroy()
				if GameXsanDestroyed then
					return
				end
				if not Success then
					...
					print("GameXsan | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with GameXsan specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= GameXsanLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					GameXsanLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			GameXsan.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("GameXsan | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with GameXsan specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					GameXsanLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("GameXsan | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with GameXsan specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					GameXsanLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("GameXsan | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with GameXsan specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					GameXsanLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= GameXsanLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("GameXsan | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with GameXsan specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						GameXsanLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("GameXsan | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with GameXsan specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					GameXsanLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	task.wait(0.1)
	TweenService:Create(Main, TweenInfo.new(0.6, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Size = useMobileSizing and UDim2.new(0, 280, 0, 160) or UDim2.new(0, 350, 0, 250)}):Play()
	TweenService:Create(Main.Shadow.Image, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {ImageTransparency = 0.6}):Play()
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			GameXsanLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			GameXsanLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('GameXsan had an issue creating settings.') end
...
function GameXsanLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function GameXsanLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function GameXsanLibrary:Destroy()
	GameXsanDestroyed = true
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['GameXsan Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[getSetting("General", "GameXsanOpen")]) and not processed then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function GameXsanLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				GameXsanLibrary:Notify({Title = "GameXsan Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			GameXsanLibrary:Notify({Title = "GameXsan Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('GameXsan Configurations Error | '..tostring(result))
			GameXsanLibrary:Notify({Title = "GameXsan Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	--local Window = GameXsanLibrary:CreateWindow({
	--	Name = "GameXsan Example Window",
	--	LoadingTitle = "GameXsan Interface Suite",
	--	Theme = 'Default',
	--	Icon = 0,
	...
	--		FileName = "Key", -- It is recommended to use something unique as other scripts using GameXsan may overwrite your key file
	--		SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
	--		GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like GameXsan to get the key from
	...
	----GameXsanLibrary:Notify({Title = "GameXsan Interface", Content = "Welcome to GameXsan. These - are the brand new notification design for GameXsan, with custom sizing and GameXsan calculated wait times.", Image = 4483362458})
	...
	--local thoptions = {}
	--for themename, theme in pairs(GameXsanLibrary.Theme) do
...
task.delay(4, function()
	GameXsanLibrary.LoadConfiguration()
...
GameXsanLibrary.CreateSafeAutoFish = function(config)
	config = config or {}
