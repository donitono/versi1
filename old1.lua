	...
	-- Safe remote event declarations with error handling
	...
	-- Safe folder access with error handling
	...
	_G.FishingDelay = 0.4 -- Default delay 0.4 seconds
	_G.HybridMode = false -- Hybrid mode dengan random delay
	_G.HybridMinDelay = 0.2 -- Minimum delay untuk hybrid mode
	_G.HybridMaxDelay = 0.5 -- Maximum delay untuk hybrid mode
	...
	local function restoreUI()
		isMinimized = false
		local targetSize = UDim2.new(0.541569591, 0, 0.64997077, 0) -- Ukuran asli
		...
		-- Tampilkan frame yang aktif
		showPanel("Main") -- Default ke Main
	...
	UserInputService.InputBegan:Connect(function(input, gameProcessed)
		...
		if input.KeyCode == Enum.KeyCode.Z then
			-- Toggle UI dengan tombol Z
			toggleUI()
		elseif input.KeyCode == Enum.KeyCode.M then
			-- Minimize/Restore dengan tombol M
			if isUIVisible then
				if isMinimized then
					restoreUI()
				else
	...
	-- Draggable UI (agar bisa dipindah)
	local dragging
	local dragInput
	local dragStart
	local startPos
	...
	-- Safe island teleport list creation
	if tpFolder then
		...
		print("Island teleport folder not found - some teleport features may not work")
	end
	
	index = 0
	
	-- Safe player teleport list creation
	if charFolder then
		for _, playerModel in ipairs(charFolder:GetChildren()) do
			if playerModel:IsA("Model") and playerModel.Name ~= game.Players.LocalPlayer.Name then
				print("Player found: " .. playerModel.Name)
				local btn = Instance.new("TextButton")
				...
				btn.MouseButton1Click:Connect(function()
					if playerModel:FindFirstChild("HumanoidRootPart") and game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
						game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = playerModel.HumanoidRootPart.CFrame
					else
		...
		print("Character folder not found - player teleport may not work")
	end
	...
	-- Update status color untuk AutoFish
	...
	-- Function untuk validate dan update fishing delay
	local function updateFishingDelay()
		...
		if delayValue == nil then
			-- Jika input bukan angka, kembalikan ke default
			AutoFishDelayTextBox.Text = tostring(_G.FishingDelay)
			return
		end
		
		-- Clamp value antara 0.1 dan 5.0 seconds
		if delayValue < 0.1 then
			delayValue = 0.1
		elseif delayValue > 5.0 then
			delayValue = 5.0
		end
	...
	-- Function untuk format dan suggest delay values
	local function suggestDelayValue(value)
		local suggestions = {
			["fast"] = 0.2,
			["normal"] = 0.4,
			["safe"] = 0.6,
		...
		return suggestions[value:lower()] or tonumber(value)
	end

	-- Function untuk update Hybrid Mode status
	local function updateHybridModeStatus(isActive)
		...
		-- Update delay textbox interactivity
		if isActive then
			-- Disable manual delay input saat hybrid mode aktif
			...
			-- Enable manual delay input saat hybrid mode nonaktif
	...
	-- Function untuk toggle Hybrid Mode
	...
	-- Quick delay presets (double click for quick values)
	local lastClickTime = 0
	AutoFishDelayTextBox.MouseButton1Click:Connect(function()
		local currentTime = tick()
		if currentTime - lastClickTime < 0.5 then
			-- Double click detected - cycle through presets
			local presets = {0.2, 0.4, 0.6, 1.0, 1.5}
			local currentDelay = _G.FishingDelay
			local nextIndex = 1
			
			for i, preset in ipairs(presets) do
				if preset > currentDelay then
					nextIndex = i
					break
				end
			end
			
			_G.FishingDelay = presets[nextIndex]
			AutoFishDelayTextBox.Text = tostring(presets[nextIndex])
			updateFishingDelay()
		end
		lastClickTime = currentTime
	end)

	-- Event handler untuk Hybrid Mode button
	...
	SellAllButton.MouseButton1Click:Connect(function()
		if sellAll then
			...
			print("Sell All function not available")
		end
	end)

	print("🎣 Modern Fishing Script Loading...")
	...
	if not Rs:FindFirstChild("Packages") then
	...
	print("- Double-click delay input to cycle presets")
