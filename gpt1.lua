if game.Players.LocalPlayer.PlayerGui:FindFirstChild("ZayrosFISHIT") ~= nil then
	...
	-- All logic wrapped to minimize top-level locals and avoid duplication
	local function initController(r)
		...
		local function RF(name)
			return netFolder and netFolder:FindFirstChild("RF/" .. name) or nil
		end
		local function RE(name)
			return netFolder and netFolder:FindFirstChild("RE/" .. name) or nil
		end
		local EquipRod = RE("EquipToolFromHotbar")
		local UnEquipRod = RE("UnequipToolFromHotbar")
		...
		local CancelFishing = RF("CancelFishingInputs")
		local spawnBoat = RF("SpawnBoat")
		local despawnBoat = RF("DespawnBoat")
		local sellAll = RF("SellAllItems")

		-- Workspace islands (optional)
		...
		-- UI list helpers
		local function ensureListLayout(scroller)
			if not scroller then return end
			if not scroller:FindFirstChildOfClass("UIListLayout") then
			...
			if not scroller:FindFirstChildOfClass("UIPadding") then
		...
		ensureListLayout(r.ListOfTPIsland)
		ensureListLayout(r.ListOfTpPlayer)
		ensureListLayout(r.ListOfTPEvent)
		r.MainFrame.AutomaticCanvasSize = Enum.AutomaticSize.Y
		
		r.PlayerFrame.AutomaticCanvasSize = Enum.AutomaticSize.Y
		r.Teleport.AutomaticCanvasSize = Enum.AutomaticSize.Y
		r.SpawnBoatFrame.AutomaticCanvasSize = Enum.AutomaticSize.Y

		local function clearList(scroller)
		...
		-- Rebuilders
		local function rebuildIslandList()
			clearList(r.ListOfTPIsland)
			if not tpFolder then
				...
				lbl.TextScaled = true
				lbl.Text = "Island folder not found"
				lbl.Size = UDim2.new(1, -12, 0, 28)
				lbl.Parent = r.ListOfTPIsland
				return
			end
			for _, island in ipairs(tpFolder:GetChildren()) do
				if island:IsA("BasePart") then
					...
					btn.Parent = r.ListOfTPIsland
					track(btn.MouseButton1Click:Connect(function()
						local myHRP = getHRP()
						if myHRP and island and island.Parent then
							myHRP.CFrame = island.CFrame
						end
					end))
				end
			end
		...
		local function rebuildEventList()
			clearList(r.ListOfTPEvent)
			local evtFolder = workspace:FindFirstChild("Events") or workspace:FindFirstChild("EventLocations")
			if not evtFolder then
				local lbl = Instance.new("TextLabel")
				lbl.BackgroundTransparency = 1
				lbl.TextColor3 = Color3.new(1,1,1)
				lbl.Font = Enum.Font.SourceSansBold
				lbl.TextScaled = true
				lbl.Text = "No events found"
				lbl.Size = UDim2.new(1, -12, 0, 28)
				lbl.Parent = r.ListOfTPEvent
				return
			end
			for _, node in ipairs(evtFolder:GetChildren()) do
				if node:IsA("BasePart") or node:IsA("Model") then
					...
					btn.TextColor3 = Color3.new(1,1,1)
					btn.Font = Enum.Font.SourceSansBold
					btn.Parent = r.ListOfTPEvent
					track(btn.MouseButton1Click:Connect(function()
						local myHRP = getHRP()
						local cf
						if node:IsA("BasePart") then cf = node.CFrame end
						if not cf and node:IsA("Model") then
							local p = node.PrimaryPart or node:FindFirstChild("HumanoidRootPart")
							if p then cf = p.CFrame end
						end
						if myHRP and cf then myHRP.CFrame = cf end
					end))
				end
			end
		end
		...
		-- Toggle dropdowns
		local open = { Island = false, Player = false, Event = false }
		local function closeAll()
			open.Island, open.Player, open.Event = false, false, false
			r.ListOfTPIsland.Visible = false
			r.ListOfTpPlayer.Visible = false
			r.ListOfTPEvent.Visible = false
		end
		local function toggleList(name)
			if not open[name] then
				closeAll()
				open[name] = true
				if name == "Island" then r.ListOfTPIsland.Visible = true; rebuildIslandList() end
				if name == "Player" then r.ListOfTpPlayer.Visible = true; rebuildPlayerList() end
				if name == "Event" then r.ListOfTPEvent.Visible = true; rebuildEventList() end
			else
				open[name] = false
				if name == "Island" then r.ListOfTPIsland.Visible = false end
				if name == "Player" then r.ListOfTpPlayer.Visible = false end
				if name == "Event" then r.ListOfTPEvent.Visible = false end
			end
		end

		-- Pages and title/tab UI
		local pages = { Main = r.MainFrame, Player = r.PlayerFrame, Teleport = r.Teleport, Boat = r.SpawnBoatFrame }
		local function setActiveTab(tabName)
			local normal = Color3.fromRGB(0,0,0)
			local active = Color3.fromRGB(30,80,180)
			r.MAIN.BackgroundColor3 = (tabName == "Main") and active or normal
			r.Player.BackgroundColor3 = (tabName == "Player") and active or normal
			r.TELEPORT.BackgroundColor3 = (tabName == "Teleport") and active or normal
			r.SpawnBoat.BackgroundColor3 = (tabName == "Boat") and active or normal
		end
		local function showPanel(name)
			for _, p in pairs(pages) do p.Visible = false end
			if pages[name] then pages[name].Visible = true end
			r.Tittle.Text = string.upper(name)
			setActiveTab(name)
		end
		...
		local function toggleFishing(state)
			if state then
				if not (EquipRod and UnEquipRod and RequestFishing and ChargeRod and FishingCompleted and CancelFishing) then
					warn("Fishing remotes not found")
					return
				end
				autoFishing = true
				...
				autoThread = task.spawn(function()
					...
					while autoFishing do
						local ok = pcall(function()
							...
							if not c:FindFirstChild("!!!EQUIPPED_TOOL!!!") then
						...
						if ok then backoff = 0.6 else backoff = math.min(backoff + 0.2, 2) end
						task.wait(backoff)
					end
					autoThread = nil
				end)
				...
				autoFishing = false
				pcall(function() CancelFishing:InvokeServer() end)
				pcall(function() UnEquipRod:FireServer() end)
			end
		end

		-- No oxygen damage (local approach)
		local oxygenEnabled = false
		local oxygenConn
		local function toggleNoOxygen()
			oxygenEnabled = not oxygenEnabled
			if oxygenConn then oxygenConn:Disconnect() oxygenConn = nil end
			if oxygenEnabled then
				oxygenConn = track(RunService.Heartbeat:Connect(function()
					local char = LocalPlayer.Character
					if not char then return end
					local oxy = char:FindFirstChild("Oxygen", true)
					if oxy and oxy:IsA("NumberValue") then oxy.Value = math.max(oxy.Value, 100) end
				end))
			end
			r.NoOxygenButton.Text = oxygenEnabled and "ON" or "OFF"
		end
		...
		local BoatIds = {
			...
			FrozenBoat = "Frozen",
			CruiserBoat = "Cruiser",
			AlphaFloaty = "AlphaFloaty",
			EvilDuck = "EvilDuck",
			FestiveDuck = "FestiveDuck",
			SantaSleigh = "SantaSleigh",
		}
		local function spawnBoatByKey(key)
			if not spawnBoat then warn("SpawnBoat RF missing") return end
			local id = BoatIds[key] or key
			pcall(function() spawnBoat:InvokeServer(id) end)
		end

		-- Button handlers
		track(r.AutoFishButton.MouseButton1Click:Connect(function()
			if autoFishing then
				toggleFishing(false)
				r.AutoFishButton.Text = "OFF"
				r.AutoFishWarna.BackgroundColor3 = Color3.fromRGB(0,0,0)
			else
				toggleFishing(true)
				r.AutoFishButton.Text = "ON"
				r.AutoFishWarna.BackgroundColor3 = Color3.fromRGB(34,139,34)
			end
		end))

		track(r.ExitBtn.MouseButton1Click:Connect(function()
			toggleFishing(false)
			unlimitedJump = false
			if oxygenConn then oxygenConn:Disconnect() oxygenConn = nil end
			disconnectAll()
			pcall(function()
				local g = LocalPlayer.PlayerGui:FindFirstChild("ZayrosFISHIT")
				if g then g:Destroy() end
			end)
		end))

		track(r.MAIN.MouseButton1Click:Connect(function() showPanel("Main") end))
		track(r.Player.MouseButton1Click:Connect(function() showPanel("Player") end))
		track(r.TELEPORT.MouseButton1Click:Connect(function() showPanel("Teleport") end))
		track(r.SpawnBoat.MouseButton1Click:Connect(function() showPanel("Boat") end))

		track(r.NoOxygenButton.MouseButton1Click:Connect(function() toggleNoOxygen() end))
		track(r.UnlimitedJumpButton.MouseButton1Click:Connect(function()
			unlimitedJump = not unlimitedJump
			r.UnlimitedJumpButton.Text = unlimitedJump and "ON" or "OFF"
			r.UnlimitedJumpWarna.BackgroundColor3 = unlimitedJump and Color3.fromRGB(34,139,34) or Color3.fromRGB(0,0,0)
		end))

		track(r.SetWalkSpeedButton.MouseButton1Click:Connect(function()
			local hum = getHumanoid()
			local num = tonumber(r.WalkSpeedTextBox.Text)
			if hum and num and num > 0 then hum.WalkSpeed = math.clamp(num, 4, 200) end
		end))

		-- Teleport toggles
		track(r.TPIslandButton.MouseButton1Click:Connect(function() toggleList("Island") end))
		track(r.TPPlayerButton.MouseButton1Click:Connect(function() toggleList("Player") end))
		track(r.TPEventButton.MouseButton1Click:Connect(function() toggleList("Event") end))

		-- Sell all
		track(r.SellAllButton.MouseButton1Click:Connect(function()
			if sellAll then pcall(function() sellAll:InvokeServer() end) end
		end))

		-- Boat buttons
		track(r.DespawnBoatButton.MouseButton1Click:Connect(function()
			if despawnBoat then pcall(function() despawnBoat:InvokeServer() end) end
		end))
		track(r.SmallBoatButton.MouseButton1Click:Connect(function() spawnBoatByKey("SmallBoat") end))
		track(r.KayakBoatButton.MouseButton1Click:Connect(function() spawnBoatByKey("KayakBoat") end))
		track(r.JetskiBoatButton.MouseButton1Click:Connect(function() spawnBoatByKey("JetskiBoat") end))
		track(r.HighfieldBoatButton.MouseButton1Click:Connect(function() spawnBoatByKey("HighfieldBoat") end))
		track(r.SpeedBoatButton.MouseButton1Click:Connect(function() spawnBoatByKey("SpeedBoat") end))
		track(r.FishingBoatButton.MouseButton1Click:Connect(function() spawnBoatByKey("FishingBoat") end))
		track(r.MiniYachtButton.MouseButton1Click:Connect(function() spawnBoatByKey("MiniYacht") end))
		track(r.HyperBoatButton.MouseButton1Click:Connect(function() spawnBoatByKey("HyperBoat") end))
		track(r.FrozenBoatButton.MouseButton1Click:Connect(function() spawnBoatByKey("FrozenBoat") end))
		track(r.CruiserBoatButton.MouseButton1Click:Connect(function() spawnBoatByKey("CruiserBoat") end))
		track(r.AlphaFloatyButton.MouseButton1Click:Connect(function() spawnBoatByKey("AlphaFloaty") end))
		track(r.EvilDuckButton.MouseButton1Click:Connect(function() spawnBoatByKey("EvilDuck") end))
		track(r.FestiveDuckButton.MouseButton1Click:Connect(function() spawnBoatByKey("FestiveDuck") end))
		track(r.SantaSleighButton.MouseButton1Click:Connect(function() spawnBoatByKey("SantaSleigh") end))

		-- Live updates for players
		track(Players.PlayerAdded:Connect(rebuildPlayerList))
		track(Players.PlayerRemoving:Connect(rebuildPlayerList))
	...
	initController({
		...
		Teleport = Teleport,
		SpawnBoatFrame = SpawnBoatFrame,
		AutoFishButton = AutoFishButton,
		AutoFishWarna = AutoFishWarna,
		NoOxygenButton = NoOxygenButton,
		UnlimitedJumpButton = UnlimitedJumpButton,
		UnlimitedJumpWarna = UnlimitedJumpWarna,
		WalkSpeedTextBox = WalkSpeedTextBox,
		SetWalkSpeedButton = SetWalkSpeedButton,
		TPIslandButton = TPIslandButton,
		TPPlayerButton = TPPlayerButton,
		TPEventButton = TPEventButton,
		ListOfTPIsland = ListOfTPIsland,
		ListOfTpPlayer = ListOfTpPlayer,
		ListOfTPEvent = ListOfTPEvent,
		SellAllButton = SellAllButton,
		DespawnBoatButton = DespawnBoatButton,
		SmallBoatButton = SmallBoatButton,
		KayakBoatButton = KayakBoatButton,
		JetskiBoatButton = JetskiBoatButton,
		HighfieldBoatButton = HighfieldBoatButton,
		SpeedBoatButton = SpeedBoatButton,
		FishingBoatButton = FishingBoatButton,
		MiniYachtButton = MiniYachtButton,
		HyperBoatButton = HyperBoatButton,
		FrozenBoatButton = FrozenBoatButton,
		CruiserBoatButton = CruiserBoatButton,
		AlphaFloatyButton = AlphaFloatyButton,
		EvilDuckButton = EvilDuckButton,
		FestiveDuckButton = FestiveDuckButton,
		SantaSleighButton = SantaSleighButton,
		ExitBtn = ExitBtn,
	})
end
