		...
		if island:IsA("BasePart") then
			...
			btn.MouseButton1Click:Connect(function()
				local success = pcall(function()
					...
					local safePosition = island.CFrame.Position + Vector3.new(0, 5, 0)
		...
		if targetPlayer:IsA("Model") and targetPlayer.Name ~= game.Players.LocalPlayer.Name then
			...
			btn.MouseButton1Click:Connect(function()
				local success = pcall(function()
					...
					local safeOffset = Vector3.new(3, 2, 3) -- Slight offset to avoid collision
					local safeCFrame = CFrame.new(targetPos + safeOffset)
				...
				while _G.AutoFishing do
					local success = pcall(function()
						...
						if hasValidRod() then
							...
							task.wait(0.5) -- Slightly longer wait
					...
					if not success then
						...
						warn("🔴 AutoFishing error, attempt " .. fishingRetryCount .. "/" .. maxRetries)
						
						if fishingRetryCount >= maxRetries then
						...
						task.wait(3) -- Wait longer on error
					else
						task.wait(1) -- Normal delay between cycles
					end
				end
	...
	function safeSpawnBoat(boatType)
		local success = pcall(function()
			...
			if currentBoat then
				...
				task.wait(0.5) -- Wait for despawn
			end
			
			print("🚢 Spawning boat: " .. boatType)
